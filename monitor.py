...
while True:
	...
	if 100 <= request_httpAdress.status_code < 200:
		 request_httpAdress_resultado = f"STATUS CODE Respostas de informação {request_httpAdress.status_code} {datetime.now()}"
	
	elif 200 <= request_httpAdress.status_code < 300:
		request_httpAdress_resultado = f"STATUS CODE Sucesso {request_httpAdress.status_code} {datetime.now()}"

	elif 300 <= request_httpAdress.status_code < 400:
		request_httpAdress_resultado = f"STATUS CODE Redirecionamento {request_httpAdress.status_code} {datetime.now()}"

	elif 400 <= request_httpAdress.status_code < 500:
		request_httpAdress_resultado = f"STATUS CODE Error cliente {request_httpAdress.status_code} {datetime.now()}"
	
	elif 500 <= request_httpAdress.status_code < 600:
		request_httpAdress_resultado = f"STATUS CODE Error servidor {request_httpAdress.status_code} {datetime.now()}"
